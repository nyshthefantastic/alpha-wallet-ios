...
final class JsonFromTokenUri {
    ...
    func fetchJsonFromTokenUri(for tokenId: String,
                               ...
                               enjinToken: GetEnjinTokenQuery.Data.EnjinToken?) -> Publisher {
        return Just(tokenId)
            .receive(on: queue)
            ...
            .flatMap { [weak self, queue, weak getTokenUri] tokenId -> AnyPublisher<NonFungibleBalanceAndItsSource<JsonString>, SessionTaskError> in
                ...
                if let promise = strongSelf.inFlightPromises[key] {
                    ...
                    let promise = getTokenUri.getUriOrTokenUri(for: tokenId, contract: address)
                        .flatMap { strongSelf.handleUriData(data: $0, tokenId: tokenId, tokenType: tokenType, address: address, enjinToken: enjinToken) }
                        ...
                        .share()
//                        .print("xxx.getTokenUri.getUriOrTokenUri")
                        .eraseToAnyPublisher()
    ...
    private func handleUriData(data: TokenUriData,
        ...
        switch data {
        case .uri(let uri):
            return fetchTokenJson(for: tokenId, tokenType: tokenType, uri: uri, address: address, enjinToken: enjinToken)
        case .string(let str):
            return generateTokenJsonFallback(for: tokenId, tokenType: tokenType, address: address)
        case .json(let json):
            do {
                let value = try fulfill(json: json, tokenId: tokenId, tokenType: tokenType, uri: nil, address: address, enjinToken: enjinToken)
                return .just(value)
            } catch {
    ...
    private func fulfill(json: JSON,
                         ...
                         uri originalUri: URL?,
                         address: AlphaWallet.Address,
                         enjinToken: GetEnjinTokenQuery.Data.EnjinToken?) throws -> NonFungibleBalanceAndItsSource<JsonString> {
        ...
        return networkService
            ...
            .flatMap { data -> Publisher in
                if let json = try? JSON(data: data.data) {
                    do {
                        return .just(try self.fulfill(json: json, tokenId: tokenId, tokenType: tokenType, uri: uri, address: address, enjinToken: enjinToken))
                    } catch {
